...
- counts2_bowtie.csv (generated in 05_Viral_Mapping.sh)
- cress_annotations.csv (determined from SSN outputs; provided in DATA section of this GitHub)

Output Notes: 
Will assess abundance and stability of the identified human skin associated viral genomes.
Generates the heatmaps shown in Fig 2A and 2B in manuscript.  
...
#counts2_bowtie.txt was opened in excel and saved as a .csv for ease in importing in R
SMA_counts <- read.csv("counts2_bowtie.csv", quote = "", header = TRUE, stringsAsFactors = FALSE, row.names = 1)
SMA_counts  <- subset(SMA_counts , select = -c(length))
```
Import taxonomy information
```{r,message=FALSE,warning=FALSE,results="hide"}
tax_ann <- read.csv("sma3_annotation.csv", quote = "", header = TRUE, stringsAsFactors = FALSE, row.names = 1)
```
## Prevalence and Stability Heatmap

Merge the identified Cress contigs taxonomy file and the count abundance table of reads mapped
```{r,message=FALSE,warning=FALSE,results="hide"}
OTU <- SMA_counts
OTU_SUMS <- merge(tax_ann, OTU, by.x = "row.names", by.y = "row.names")
OTU_SUMS<- subset (OTU_SUMS, select = -c(Row.names))
```
...
```{r,message=FALSE,warning=FALSE,results="hide"}
OTU_SUMS2<- subset (OTU_SUMS, select = -c(Type,Superkingdom,Realm,Kingdom,Phylum,Class,Order))
OTU_SUMS2<-ddply(OTU_SUMS2,"Isolate",numcolwise(max))
OTU_SUMS2<- subset (OTU_SUMS2, select = -c(HV_015_20, HV_015_21,HV_027_02, HV_027_03, HV_027_04, HV_027_05, HV_027_06, HV_027_07, HV_020_10, HV_021_13, HV_027_01))
tax_ann2<- subset (tax_ann, select = -c(Type,Superkingdom,Realm,Kingdom,Phylum,Class,Order))
OTU_SUMS2$Family<-"Unclassified"
Sma_list<-tax_ann2$Isolate[which(tax_ann2$Family=="Smacoviridae")]
OTU_SUMS2$Family[which(OTU_SUMS2$Isolate %in% Sma_list)]<-"Smacoviridae"

geno_list<-tax_ann2$Isolate[which(tax_ann2$Family=="Genomoviridae")]
OTU_SUMS2$Family[which(OTU_SUMS2$Isolate %in% geno_list)]<-"Genomoviridae"

circ_list<-tax_ann2$Isolate[which(tax_ann2$Family=="Circoviridae")]
OTU_SUMS2$Family[which(OTU_SUMS2$Isolate %in% circ_list)]<-"Circoviridae"

UF1_list<-tax_ann2$Isolate[which(tax_ann2$Family=="Unclassified family 1")]
OTU_SUMS2$Family[which(OTU_SUMS2$Isolate %in% UF1_list)]<-"Unclassified family 1"

UF2_list<-tax_ann2$Isolate[which(tax_ann2$Family=="Unclassified family 2")]
OTU_SUMS2$Family[which(OTU_SUMS2$Isolate %in% UF2_list)]<-"Unclassified family 2"

UF3_list<-tax_ann2$Isolate[which(tax_ann2$Family=="Unclassified family 3")]
OTU_SUMS2$Family[which(OTU_SUMS2$Isolate %in% UF3_list)]<-"Unclassified family 3"

UF4_list<-tax_ann2$Isolate[which(tax_ann2$Family=="Unclassified family 4")]
OTU_SUMS2$Family[which(OTU_SUMS2$Isolate %in% UF4_list)]<-"Unclassified family 4"

UF5_list<-tax_ann2$Isolate[which(tax_ann2$Family=="Unclassified family 5")]
OTU_SUMS2$Family[which(OTU_SUMS2$Isolate %in% UF5_list)]<-"Unclassified family 5"

UF6_list<-tax_ann2$Isolate[which(tax_ann2$Family=="Unclassified family 6")]
OTU_SUMS2$Family[which(OTU_SUMS2$Isolate %in% UF6_list)]<-"Unclassified family 6"

UF7_list<-tax_ann2$Isolate[which(tax_ann2$Family=="Unclassified family 7")]
OTU_SUMS2$Family[which(OTU_SUMS2$Isolate %in% UF7_list)]<-"Unclassified family 7"


OTU_SUMS2<- subset (OTU_SUMS2, select = -c(Isolate))
OTU_SUMS3<-ddply(OTU_SUMS2,"Family",numcolwise(sum))
```

Convert abundance into binary presence/absence 
```{r,message=FALSE,warning=FALSE,results="hide"}
OTU_SUMS3 -> OTU_Stable 
OTU_Stable <- OTU_Stable %>% mutate_if(is.numeric, ~1 * (. != 0))
```

Determine prevalence over the five time points by location within a subject
```{r,message=FALSE,warning=FALSE,results="hide"}
#P01 Left
...
#P38 Right
OTU_Stable$P38_RH <- OTU_Stable$HV_026_16 + OTU_Stable$HV_028_16 + OTU_Stable$HV_029_16 + OTU_Stable$HV_030_15 + OTU_Stable$HV_031_19 
OTU_Stable <- subset(OTU_Stable, select = -c(HV_026_16,HV_028_16,HV_029_16,HV_030_15, HV_031_19))
#P38 Scalp
...
#P39 Right
OTU_Stable$P39_RH <- OTU_Stable$HV_026_18 + OTU_Stable$HV_028_18 + OTU_Stable$HV_029_18 + OTU_Stable$HV_030_16 + OTU_Stable$HV_031_20
OTU_Stable <- subset(OTU_Stable, select = -c(HV_026_18,HV_028_18,HV_029_18,HV_030_16,HV_031_20))
#P39 Scalp
...
#P40 Right
OTU_Stable$P40_RH <- OTU_Stable$HV_026_19 + OTU_Stable$HV_028_19 + OTU_Stable$HV_029_19 + OTU_Stable$HV_030_18 + OTU_Stable$HV_031_21
OTU_Stable <- subset(OTU_Stable, select = -c(HV_026_19,HV_028_19,HV_029_19,HV_030_18,HV_031_21))
#P40 Scalp
...
#P41 Right
OTU_Stable$P41_RH <- OTU_Stable$HV_026_20 + OTU_Stable$HV_028_20 + OTU_Stable$HV_029_20 + OTU_Stable$HV_030_19 + OTU_Stable$HV_031_22
OTU_Stable <- subset(OTU_Stable, select = -c(HV_026_20,HV_028_20,HV_029_20,HV_030_19,HV_031_22))
#P41 Scalp
...
#P42 Left
OTU_Stable$P42_LH <- OTU_Stable$HV_022_21 + OTU_Stable$HV_023_21 + OTU_Stable$HV_024_21 + OTU_Stable$HV_025_21 + OTU_Stable$HV_031_01
OTU_Stable <- subset(OTU_Stable, select = -c(HV_022_21,HV_023_21,HV_024_21,HV_025_21,HV_031_01))
#P42 Right
...
#P44 Left
OTU_Stable$P44_LH <- OTU_Stable$HV_031_02 + OTU_Stable$HV_032_01 + OTU_Stable$HV_033_01 + OTU_Stable$HV_034_01
OTU_Stable <- subset(OTU_Stable, select = -c(HV_031_02,HV_032_01,HV_033_01,HV_034_01))
#P44 Right
OTU_Stable$P44_RH <- OTU_Stable$HV_032_02 + OTU_Stable$HV_033_02 + OTU_Stable$HV_034_02
OTU_Stable <- subset(OTU_Stable, select = -c(HV_032_02,HV_034_02,HV_033_02))
#P44 Scalp
OTU_Stable$P44_SC <- OTU_Stable$HV_032_03 + OTU_Stable$HV_033_03 + OTU_Stable$HV_034_03 +OTU_Stable$HV_020_22
OTU_Stable <- subset(OTU_Stable, select = -c(HV_032_03,HV_033_03,HV_034_03,HV_020_22))
#P46 Left
OTU_Stable$P46_LH <- OTU_Stable$HV_031_03 + OTU_Stable$HV_032_04 + OTU_Stable$HV_034_04
OTU_Stable <- subset(OTU_Stable, select = -c(HV_031_03,HV_032_04,HV_034_04))
#P46 Right
OTU_Stable$P46_RH <- OTU_Stable$HV_033_05 + OTU_Stable$HV_032_05 + OTU_Stable$HV_034_05
OTU_Stable <- subset(OTU_Stable, select = -c(HV_032_05,HV_033_05,HV_034_05))
#P46 Scalp
OTU_Stable$P46_SC <- OTU_Stable$HV_032_06 + OTU_Stable$HV_033_06 + OTU_Stable$HV_034_06
OTU_Stable <- subset(OTU_Stable, select = -c(HV_032_06,HV_033_06,HV_034_06))
#P47 Left
OTU_Stable$P47_LH <- OTU_Stable$HV_031_04 + OTU_Stable$HV_032_07 + OTU_Stable$HV_033_07 + OTU_Stable$HV_034_07
OTU_Stable <- subset(OTU_Stable, select = -c(HV_031_04,HV_032_07,HV_033_07,HV_034_07))
#P47 Right
OTU_Stable$P47_RH <- OTU_Stable$HV_032_08 + OTU_Stable$HV_033_08 + OTU_Stable$HV_034_08
OTU_Stable <- subset(OTU_Stable, select = -c(HV_032_08, HV_033_08,HV_034_08))
#P47 Scalp
OTU_Stable$P47_SC <- OTU_Stable$HV_032_09 + OTU_Stable$HV_033_09 + OTU_Stable$HV_034_09
OTU_Stable <- subset(OTU_Stable, select = -c(HV_032_09,HV_033_09,HV_034_09))
#P48 Left
OTU_Stable$P48_LH <- OTU_Stable$HV_031_05 + OTU_Stable$HV_032_10 + OTU_Stable$HV_033_10 + OTU_Stable$HV_034_10
OTU_Stable <- subset(OTU_Stable, select = -c(HV_031_05,HV_032_10,HV_033_10,HV_034_10))
#P48 Right
OTU_Stable$P48_RH <- OTU_Stable$HV_032_11 + OTU_Stable$HV_033_11 + OTU_Stable$HV_034_11
OTU_Stable <- subset(OTU_Stable, select = -c(HV_032_11,HV_033_11,HV_034_11))
#P48 Scalp
OTU_Stable$P48_SC <- OTU_Stable$HV_032_12 + OTU_Stable$HV_034_12
OTU_Stable <- subset(OTU_Stable, select = -c(HV_032_12,HV_034_12))
#P49 Left
OTU_Stable$P49_LH <- OTU_Stable$HV_031_06 + OTU_Stable$HV_032_13 + OTU_Stable$HV_033_13
OTU_Stable <- subset(OTU_Stable, select = -c(HV_031_06,HV_032_13,HV_033_13))
#P49 Right
OTU_Stable$P49_RH <- OTU_Stable$HV_032_14 + OTU_Stable$HV_033_14
OTU_Stable <- subset(OTU_Stable, select = -c(HV_032_14,HV_033_14))
#P49 Scalp
OTU_Stable$P49_SC <- OTU_Stable$HV_032_15 + OTU_Stable$HV_033_15 + OTU_Stable$HV_034_15
OTU_Stable <- subset(OTU_Stable, select = -c(HV_032_15,HV_033_15,HV_034_15))
#P50 Left
OTU_Stable$P50_LH <- OTU_Stable$HV_031_07 + OTU_Stable$HV_032_16 + OTU_Stable$HV_033_16 + OTU_Stable$HV_034_16
OTU_Stable <- subset(OTU_Stable, select = -c(HV_031_07,HV_032_16,HV_033_16,HV_034_16))
#P50 Right
OTU_Stable$P50_RH <- OTU_Stable$HV_032_18 + OTU_Stable$HV_033_18 + OTU_Stable$HV_034_18
OTU_Stable <- subset(OTU_Stable, select = -c(HV_032_18,HV_033_18,HV_034_18))
#P50 Scalp
OTU_Stable$P50_SC <- OTU_Stable$HV_032_19 + OTU_Stable$HV_033_19 + OTU_Stable$HV_034_19
OTU_Stable <- subset(OTU_Stable, select = -c(HV_032_19,HV_033_19,HV_034_19))
#P51 Left
OTU_Stable$P51_LH <- OTU_Stable$HV_031_08 + OTU_Stable$HV_032_20 + OTU_Stable$HV_033_20 + OTU_Stable$HV_034_20
OTU_Stable <- subset(OTU_Stable, select = -c(HV_031_08,HV_032_20,HV_033_20,HV_034_20))
#P51 Right
OTU_Stable$P51_RH <- OTU_Stable$HV_032_21 + OTU_Stable$HV_033_21 + OTU_Stable$HV_034_21
OTU_Stable <- subset(OTU_Stable, select = -c(HV_032_21,HV_033_21,HV_034_21))
#P51 Scalp
OTU_Stable$P51_SC <- OTU_Stable$HV_032_22 + OTU_Stable$HV_033_22 + OTU_Stable$HV_034_22
OTU_Stable <- subset(OTU_Stable, select = -c(HV_032_22,HV_033_22,HV_034_22))
#P52 Left
OTU_Stable$P52_LH <- OTU_Stable$HV_031_09 
OTU_Stable <- subset(OTU_Stable, select = -c(HV_031_09))
#P53 Left
OTU_Stable$P53_LH <- OTU_Stable$HV_031_10
OTU_Stable <- subset(OTU_Stable, select = -c(HV_031_10))
#P54 Left
OTU_Stable$P54_LH <- OTU_Stable$HV_031_11
OTU_Stable <- subset(OTU_Stable, select = -c(HV_031_11))
#P55 Left
OTU_Stable$P55_LH <- OTU_Stable$HV_031_12
OTU_Stable <- subset(OTU_Stable, select = -c(HV_031_12))
#P56 Left
OTU_Stable$P56_LH <- OTU_Stable$HV_031_13
OTU_Stable <- subset(OTU_Stable, select = -c(HV_031_13))
#P57 Left
OTU_Stable$P57_LH <- OTU_Stable$HV_031_14
OTU_Stable <- subset(OTU_Stable, select = -c(HV_031_14))
#P58 Left
OTU_Stable$P58_LH <- OTU_Stable$HV_031_15
OTU_Stable <- subset(OTU_Stable, select = -c(HV_031_15))
#P59 Left
OTU_Stable$P59_LH <- OTU_Stable$HV_031_16
OTU_Stable <- subset(OTU_Stable, select = -c(HV_031_16))
#P60 Left
OTU_Stable$P60_LH <- OTU_Stable$HV_031_18
OTU_Stable <- subset(OTU_Stable, select = -c(HV_031_18))
OTU_Stable_time<-OTU_Stable
```

Split data into two matrix for heatmap generation
```{r}
OTU_Stable_time$Family->row.names(OTU_Stable_time)
OTU_Stable_time<- subset (OTU_Stable_time, select = -c(Family))
OTU_Stable_time2<- subset (OTU_Stable_time, select = -c(P44_LH,P44_RH,P44_SC,P46_LH,P46_RH,P46_SC,P47_LH,P47_RH,P47_SC,P48_LH,P48_RH,P48_SC,P49_LH,P49_RH,P49_SC,P50_LH,P50_RH,P50_SC,P51_LH,P51_RH,P51_SC,P52_LH,P53_LH,P54_LH,P55_LH,P56_LH,P57_LH,P58_LH,P59_LH,P60_LH))

OTU_Stable_time2_1<-subset (OTU_Stable_time2, select = c(P01_LH, P01_RH, P01_SC, P02_LH, P02_RH, P02_SC, P03_LH, P03_RH, P03_SC, P04_LH, P04_RH, P04_SC, P05_LH, P05_RH, P05_SC, P06_LH, P06_RH, P06_SC, P07_LH, P07_RH, P07_SC, P08_LH, P08_RH, P08_SC, P09_LH, P09_RH, P09_SC, P10_LH, P10_RH, P10_SC, P11_LH, P11_RH, P11_SC, P12_LH, P12_RH, P12_SC, P13_LH, P13_RH, P13_SC, P14_LH, P14_RH, P14_SC, P15_LH, P15_RH, P15_SC, P16_LH, P16_RH, P16_SC, P17_LH, P17_RH, P17_SC, P18_LH, P18_RH, P18_SC, P19_LH, P19_RH, P19_SC, P20_LH, P20_RH, P20_SC,P21_LH, P21_RH, P21_SC))

OTU_Stable_time2_2<-subset (OTU_Stable_time2, select = c(P22_LH, P22_RH, P22_SC, P23_LH, P23_RH, P23_SC, P24_LH, P24_RH, P24_SC, P25_LH, P25_RH, P25_SC, P26_LH, P26_RH, P26_SC, P27_LH, P27_RH, P27_SC, P28_LH, P28_RH, P28_SC, P29_LH, P29_RH, P29_SC, P30_LH, P30_RH, P30_SC, P31_LH, P31_RH, P31_SC, P32_LH, P32_RH, P32_SC, P34_LH, P34_RH, P34_SC, P35_LH, P35_RH, P35_SC, P36_LH, P36_RH, P36_SC, P37_LH, P37_RH, P37_SC, P38_LH, P38_RH, P38_SC, P39_LH, P39_RH, P39_SC, P40_LH, P40_RH, P40_SC, P41_LH, P41_RH, P41_SC, P42_LH, P42_RH, P42_SC,P43_LH, P43_RH, P43_SC))


profile2<-as.matrix(OTU_Stable_time2)
profile21<-as.matrix(OTU_Stable_time2_1)
```
...
pheatmaps.obj<-pheatmap(profile2, 
                        cluster_cols = FALSE,
                        cluster_rows=FALSE, 
                        gaps_col = c(3,6,9,12,15,18,21,24,27,30,33,36,39,42,45,48,51,54,57,60,63,66,69,72,75,78,81,84,87,90,93,96,99,102,105,108,111,114,117,120,123,126),
                        cellheight = 11,
                        cellwidth = 11,
                        fontsize = 12, 
                        filename = "Heatmap_time_all.pdf")

pheatmaps.obj<-pheatmap(profile22, 
                        cluster_cols = FALSE,
                        cluster_rows=FALSE, 
                        gaps_col = c(3,6,9,12,15,18,21,24,27,30,33,36,39,42,45,48,51,54,57,60,63),
                        cellheight = 11,
                        cellwidth = 11,
                        fontsize = 12, 
                        filename = "Heatmap_time_all2.pdf")
...
OTU_Stable$P60_LH <- OTU_Stable$HV_031_18
OTU_Stable <- subset(OTU_Stable, select = -c(HV_031_18))
OTU_Stable_abund<-OTU_Stable
```

Generate Heatmap
```{r}
OTU_Stable_abund$Family->row.names(OTU_Stable_abund)
OTU_Stable_abund<- subset (OTU_Stable_abund, select = -c(Family))
OTU_Stable_abund2<- subset (OTU_Stable_abund, select = -c(P44_LH,P44_RH,P44_SC,P46_LH,P46_RH,P46_SC,P47_LH,P47_RH,P47_SC,P48_LH,P48_RH,P48_SC,P49_LH,P49_RH,P49_SC,P50_LH,P50_RH,P50_SC,P51_LH,P51_RH,P51_SC,P52_LH,P53_LH,P54_LH,P55_LH,P56_LH,P57_LH,P58_LH,P59_LH,P60_LH))
OTU_Stable_abund2log<-OTU_Stable_abund2+1
OTU_Stable_abund2log<-log10(OTU_Stable_abund2log)

OTU_Stable_abund2log_1<-subset (OTU_Stable_abund2log, select = c(P01_LH, P01_RH, P01_SC, P02_LH, P02_RH, P02_SC, P03_LH, P03_RH, P03_SC, P04_LH, P04_RH, P04_SC, P05_LH, P05_RH, P05_SC, P06_LH, P06_RH, P06_SC, P07_LH, P07_RH, P07_SC, P08_LH, P08_RH, P08_SC, P09_LH, P09_RH, P09_SC, P10_LH, P10_RH, P10_SC, P11_LH, P11_RH, P11_SC, P12_LH, P12_RH, P12_SC, P13_LH, P13_RH, P13_SC, P14_LH, P14_RH, P14_SC, P15_LH, P15_RH, P15_SC, P16_LH, P16_RH, P16_SC, P17_LH, P17_RH, P17_SC, P18_LH, P18_RH, P18_SC, P19_LH, P19_RH, P19_SC, P20_LH, P20_RH, P20_SC,P21_LH, P21_RH, P21_SC))

OTU_Stable_abund2log_2<-subset (OTU_Stable_abund2log, select = c(P22_LH, P22_RH, P22_SC, P23_LH, P23_RH, P23_SC, P24_LH, P24_RH, P24_SC, P25_LH, P25_RH, P25_SC, P26_LH, P26_RH, P26_SC, P27_LH, P27_RH, P27_SC, P28_LH, P28_RH, P28_SC, P29_LH, P29_RH, P29_SC, P30_LH, P30_RH, P30_SC, P31_LH, P31_RH, P31_SC, P32_LH, P32_RH, P32_SC, P34_LH, P34_RH, P34_SC, P35_LH, P35_RH, P35_SC, P36_LH, P36_RH, P36_SC, P37_LH, P37_RH, P37_SC, P38_LH, P38_RH, P38_SC, P39_LH, P39_RH, P39_SC, P40_LH, P40_RH, P40_SC, P41_LH, P41_RH, P41_SC, P42_LH, P42_RH, P42_SC,P43_LH, P43_RH, P43_SC))

profile<-as.matrix(OTU_Stable_abund2log)
pheatmaps.obj<-pheatmap(profile, 
                        cluster_cols = FALSE,
                        cluster_rows=FALSE, 
                        gaps_col = c(3,6,9,12,15,18,21,24,27,30,33,36,39,42,45,48,51,54,57,60,63,66,69,72,75,78,81,84,87,90,93,96,99,102,105,108,111,114,117,120,123,126),
                        ...
                        cellwidth = 11,
                        fontsize = 12, 
                        filename = "Heatmap_abund_all.pdf")
profile11<-as.matrix(OTU_Stable_abund2log_1)
pheatmaps.obj<-pheatmap(profile11, 
                        cluster_cols = FALSE,
                        cluster_rows=FALSE, 
                        gaps_col = c(3,6,9,12,15,18,21,24,27,30,33,36,39,42,45,48,51,54,57,60,63),
                         color = hcl.colors(50, "BluYl"),
                        ...
                        cellwidth = 11,
                        fontsize = 12, 
                        filename = "Heatmap_abund_all1.pdf")
profile12<-as.matrix(OTU_Stable_abund2log_2)
pheatmaps.obj<-pheatmap(profile12, 
                        cluster_cols = FALSE,
                        cluster_rows=FALSE, 
                        gaps_col = c(3,6,9,12,15,18,21,24,27,30,33,36,39,42,45,48,51,54,57,60,63),
                         color = hcl.colors(50, "BluYl"),
